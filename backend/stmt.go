...
func (self *Stmt) Execute(args ...interface{}) (*mysql.Result, error) {
	if err := self.write(args...); err != nil {
		return nil, err
	}

	return self.conn.readResult(true)
}

func (self *Stmt) Close() error {
	if err := self.conn.writeCommandUint32(mysql.COM_STMT_CLOSE, self.id); err != nil {
		return err
	}
...
func (self *Stmt) write(args ...interface{}) error {
	paramsNum := self.params

	if len(args) != paramsNum {
		return fmt.Errorf("argument mismatch, need %d but got %d", self.params, len(args))
	}
	...
	for i := range args {
		...
		case int16:
			...
			paramValues[i] = utils.Uint16ToBytes(uint16(v))
		case int32:
			paramTypes[i<<1] = mysql.MYSQL_TYPE_LONG
			paramValues[i] = utils.Uint32ToBytes(uint32(v))
		case int:
			paramTypes[i<<1] = mysql.MYSQL_TYPE_LONGLONG
			paramValues[i] = utils.Uint64ToBytes(uint64(v))
		case int64:
			paramTypes[i<<1] = mysql.MYSQL_TYPE_LONGLONG
			paramValues[i] = utils.Uint64ToBytes(uint64(v))
		case uint8:
		...
		case float32:
			...
			paramValues[i] = utils.Uint32ToBytes(math.Float32bits(v))
		case float64:
			paramTypes[i<<1] = mysql.MYSQL_TYPE_DOUBLE
			paramValues[i] = utils.Uint64ToBytes(math.Float64bits(v))
		case string:
	...
	data = append(data, byte(self.id), byte(self.id>>8), byte(self.id>>16), byte(self.id>>24))
	...
	self.conn.pkg.Sequence = 0

	return self.conn.pkg.WritePacket(data)
}

func (self Conn) Prepare(query string) (*Stmt, error) {
	if err := self.writeCommandStr(mysql.COM_STMT_PREPARE, query); err != nil {
		return nil, err
	}
