...
func (self *Session) buildResultset(names []string, values [][]interface{}) (*mysql.Resultset, error) {
	...
	for i, vs := range values {
		...
		for j, value := range vs {
			...
			row = append(row, utils.PutLengthEncodedString(b)...)
		}
...
func (self *Session) writeResultset(status uint16, r *mysql.Resultset) error {
	self.affectedRows = int64(-1)

	columnLen := utils.PutLengthEncodedInt(uint64(len(r.Fields)))

	data := make([]byte, 4, 1024)

	data = append(data, columnLen...)
	if err := self.pkg.WritePacket(data); err != nil {
		return err
	}
	...
	if err := self.writeEOF(status); err != nil {
		return err
	}
